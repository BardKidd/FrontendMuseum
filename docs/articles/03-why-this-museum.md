...
1. **親手按。**站在 <https://frontendmuseum.pages.dev> ——
   首頁是標本索引，量測台在 `/measure.html`。每個標本頁都能切病變／治療，面板即時顯示指標。
...
- **六個標本不是效能反模式的全集**，也不打算是。六個做透勝過二十個做淺，
  是 spec 開頭就定的取捨。
- **部署本身也過了驗收才算數**：上線當天把 13 條驗收原封不動對著生產站再跑一次
  （13/13，校準錨點 304ms ×3、離散 0.0%），另實抓零外部請求與注入檢查
  （四頁、含 404 頁，全綠）。託管平台的 SPA fallback 差點把「亂打的路徑回 200」
  端上桌，被部署驗收第一條抓掉 —— 詳見 `docs/HANDOFF.md` 部署一節。
