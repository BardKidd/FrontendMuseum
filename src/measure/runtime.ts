...
export function bootstrapSpecimen(mod: SpecimenModule): void {
  ...
  function flush(): void {
    try {
      ...
      if (batch.interactions.length === 0 && !batch.customDirty && inpValue === lastInp) {
        return;
      }
      lastInp = inpValue;
      seq += 1;
      ...
      const msg: SpecimenMetrics = {
        ...
        inp: batch.inp,
        // Phase 0 完全不觀測載入期指標，欄位先存在。之後補 observer 只動 metrics.ts，
        // 不動協定、不動任何標本，先前數字不作廢（spec §5「凍結型別，不凍結實作」）。
        lcp: null,
        cls: null,
        custom: batch.custom,
        ...
        .then(({ onINP }) => {
          onINP(
            (metric) => {
              ...
              // 不是證明我完全正確（spec §5.6）。
              const mine = lastInp;
              const check: WebVitalsCrossCheck = {
                inp: metric.value,
                // Phase 0 只交叉驗 INP。LCP / CLS 這邊本來就回 null，沒有東西可以比。
                lcp: null,
                cls: null,
              ...
              console.table([
                { source: 'hand-rolled', inp: mine },
                { source: 'web-vitals', inp: metric.value },
                { source: 'delta (mine - lib)', inp: check.deltaInp },
              ]);
            },
            {
